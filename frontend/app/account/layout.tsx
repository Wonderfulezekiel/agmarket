...
export default function AccountLayout({
  ...
  if (!mounted) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-50">
        <p className="text-zinc-500 text-sm">Loading Account...</p>
      </div>
    );
  }
  ...
  if (!currentUser || currentUser.role !== 'buyer') {
    return (
      <div className="mx-auto max-w-md px-4 py-16 text-center space-y-4">
        ...
        <h2 className="font-serif text-2xl font-bold text-zinc-900">Buyer Access Required</h2>
        <p className="text-sm text-zinc-500">
          You must be logged in as a Buyer to access this profile. Use the top menu role switcher to change your role to John Doe.
        </p>
        <button
          onClick={() => {
            useAppStore.getState().switchUserRole('buyer');
            router.refresh();
          }}
          className="rounded-lg bg-emerald-600 px-6 py-2.5 text-xs font-semibold text-white hover:bg-emerald-700 shadow-sm"
        >
          Quick Switch to Buyer
        </button>
      </div>
    );
  }
  ...
  const menuItems = [
    { name: 'Profile Details', href: '/account', icon: UserIcon },
    { name: 'Purchase History', href: '/account/orders', icon: ShoppingBag },
    { name: 'Chat Messages', href: '/account/messages', icon: MessageSquare },
  ];
    ...
    <div className="min-h-screen bg-zinc-50 flex flex-col md:flex-row transition-colors">
      ...
      <div className="md:hidden flex items-center justify-between px-4 py-3 bg-white border-b border-zinc-200">
        <div className="flex items-center gap-2">
          ...
          />
          <div>
            <h2 className="text-xs font-bold text-zinc-900 truncate max-w-[150px]">{currentUser.name}</h2>
            <span className="text-[9px] font-semibold text-emerald-705 uppercase bg-emerald-50 px-1.5 rounded">Buyer</span>
          </div>
        </div>
        <button
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-zinc-250 flex-shrink-0 p-5 space-y-6">
        ...
        <div className="flex items-center gap-3 bg-zinc-50 rounded-xl p-3 border border-zinc-200">
          <img
            src={currentUser.avatar}
            alt={currentUser.name}
            className="h-10 w-10 rounded-full object-cover border border-zinc-250 bg-white"
          />
          <div className="min-w-0">
            <h2 className="text-xs font-extrabold text-zinc-900 truncate" title={currentUser.name}>
              {currentUser.name}
            </h2>
            <p className="text-[9px] text-zinc-400 font-semibold truncate mt-0.5">{currentUser.location || 'Buyer Location'}</p>
          </div>
        </div>
        ...
        <div className="md:hidden fixed inset-0 z-40 bg-black/40 backdrop-blur-xs flex">
          <div className="w-64 bg-white p-5 flex flex-col space-y-6 shadow-xl animate-slide-in h-full">
            <div className="flex justify-between items-center pb-3 border-b border-zinc-200">
              <span className="font-bold text-zinc-800 text-sm">Account Navigation</span>
