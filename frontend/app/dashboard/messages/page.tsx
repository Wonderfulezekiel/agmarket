...
export default function FarmerMessagesPage() {
  ...
  // Filter messages involving this farmer
  const farmerMessages = messages.filter(
    m => m.senderId === currentUser.id || m.receiverId === currentUser.id
  );

  // Group messages by chat partner (the other person)
  const chatPartnersMap = new Map<string, { partnerName: string; lastMessage: Message }>();
  
  farmerMessages.forEach(msg => {
    const partnerId = msg.senderId === currentUser.id ? msg.receiverId : msg.senderId;
    const partnerName = msg.senderId === currentUser.id ? msg.receiverId : msg.senderName; // Note: simplified fallback
    
    // Resolve full name from user records if possible
    const resolvedUser = users.find(u => u.id === partnerId);
    const resolvedName = resolvedUser ? resolvedUser.name : 'Buyer';
  ...
  return (
    <div className="space-y-6 h-[calc(100vh-140px)] flex flex-col">
      ...
      <div>
        ...
        <p className="text-sm text-zinc-500 mt-1">Communicate directly with buyers to discuss product inquiries, delivery logistics, and custom quotes.</p>
      </div>
      ...
      {chatPartners.length === 0 ? (
        <div className="flex-grow border border-dashed border-zinc-300 rounded-3xl p-12 text-center flex flex-col items-center justify-center space-y-4">
          ...
          <p className="text-zinc-500 text-sm font-medium">You have no active conversations right now.</p>
          <p className="text-xs text-zinc-400">Conversations will appear here when buyers contact your storefront.</p>
        </div>
      ) : (
        <div className="flex-grow grid grid-cols-1 md:grid-cols-3 border border-zinc-200 bg-white rounded-2xl overflow-hidden shadow-xs">
          ...
          <div className={`md:col-span-1 border-r border-zinc-200 flex flex-col h-full ${activePartnerId ? 'hidden md:flex' : 'flex'}`}>
            <div className="p-4 border-b border-zinc-150 bg-zinc-50">
              <h3 className="text-xs font-bold text-zinc-900 uppercase tracking-wider">Conversations</h3>
            </div>
          ...
          <div className={`md:col-span-2 flex flex-col h-full ${!activePartnerId ? 'hidden md:flex' : 'flex'}`}>
            {activePartner ? (
                ...
                <div className="p-4 border-b border-zinc-200 bg-zinc-50 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-zinc-400 font-semibold uppercase">Buyer Persona</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 flex items-center justify-center text-center p-6 text-zinc-500">
                Select a conversation to display message logs.
              </div>
            )}
          </div>
